...
class TranslatePracticeViewModel:
    ...
    def handle_step_1(self, title: str, ref_source: str, input_text: str):
        ...
        def handling():
            ...
            text_translated = self.translator.translate_eng_to_vn(input_text)
            self.paragraph_service.update_paragraph(paragraph_id=self.paragraph_id, machine_translation=text_translated)
    ...
    def handle_step_2(self, sentence_translations: list[str], new_words_list: list[str]):
        ...
        self.switch_step(TRANSLATE_PRACTICE_STEP.LOADING)
        self.process_translations(sentence_translations)
        self.process_new_words(new_words_list)
        self.switch_step(TRANSLATE_PRACTICE_STEP.STEP_3_REVIEW_TRANSLATION)
        
    def process_translations(self, sentence_translations: list[str]):
        un_complete = 0
        scores = []
        for idx, translation in enumerate(sentence_translations, start=1):
        ...
        paragraph_scored = round(sum(scores)/len(scores), 2)
    ...
    def process_new_words(self, new_words_list: list[str]):
        for idx, new_words in enumerate(new_words_list, start=1):
    ...
    def get_paragraph_data(self):
        """Load paragraph data from DB"""
        if self.paragraph_id:
            return self.paragraph_service.get_paragraph_by_id(self.paragraph_id)
        return None

    def get_sentences_detail(self):
        """Load all sentences with translations and scores from DB"""
        if self.paragraph_id:
            return self.sentence_service.get_sentences_by_paragraph_id(self.paragraph_id)
        return []
